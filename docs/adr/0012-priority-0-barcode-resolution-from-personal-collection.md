...
2. **Disambiguation & Escape Hatch**:
   - If multiple spools match the barcode with the same **Filament Type**, the most recent spool is cloned silently. If matches point to conflicting Filament Types, a candidate disambiguation dialog is shown; dismissing it leaves the barcode intact and fields blank.
   - When Priority 0 resolves a match, a toast notifies the user and offers a `"Search Community Catalog instead"` action to allow fetching fresh external specifications on demand.
